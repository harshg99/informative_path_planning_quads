...
class PrioritisedReplayBuffer(ReplayBuffer):
    def __init__(self,data_spec,capacity):
        self.buffer_keys.append('priority')
        super().__init__(data_spec,capacity)

    def get_next(self, sample_batch_size=None):
        """Returns an item or batch of items from the buffer."""
        probabilities = np.array(self.buffer['priority'])
        probabilities = probabilities/probabilities.sum()

        if sample_batch_size > self.occupied_capacity:
            sample_indices = np.random.choice(np.arange(self.occupied_capacity), sample_batch_size, p=probabilities,replace=True)
        else:
            sample_indices = np.random.choice(np.arange(self.occupied_capacity), sample_batch_size, p=probabilities,replace=False)
