...
from mcts_agent import MlpPolicyValueNet as AgentModel

from mcts_planner import mcts_planner

from mcts_treesearch import improve_policy_with_mcts, recurrent_fn
from mcts_utils import batched_policy, env_step, import_class, replicate, reset_env
...
def train(
    ...
    num_self_plays_per_iteration: int = 8 * 100,
    learning_rate: float = 0.01,
    ckpt_filename: str = "./baselines/MCTS/agent_model/agent.ckpt",
    random_seed: int = 42,
    weight_decay: float = 1e-4,
    lr_decay_steps: int = 100_000,
):
    """Train an agent by self-play."""
    env = import_class(args_dict)
    agent = AgentModel(
        input_dims=env.input_size,
...
if __name__=="__main__":
    ...
    import Utilities
    args_dict = Utilities.set_dict(arguments)
    train(args_dict)
