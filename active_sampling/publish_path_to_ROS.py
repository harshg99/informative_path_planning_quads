...
class PolicyToROS:
    def __init__(self):
        self.odom = None
        rospy.Subscriber("/quadrotor/odom", Odometry, self.odom_cb, queue_size=1)
    ...
    def traj_opt(self, tau, pg, example_mp):
        dimension = 2
        ...
        rospy.loginfo(f"starting position {[self.odom.pose.pose.position.x, self.odom.pose.pose.position.y]}")
        start.addConstraint(derivative_order.POSITION, [self.odom.pose.pose.position.x, self.odom.pose.pose.position.y])
...
if __name__ == '__main__':
    ...
    client = actionlib.SimpleActionClient('quadrotor/trackers_manager/execute_trajectory', RunTrajectoryAction)
    # listening for goals.
